    ...
    async def _func(*dice):
        ...
        if len(dice) == 1 and dice[0][0] == '1':
            ...
            if roll > die_5 * 4 + randint(-die_5, die_5):
                embed.add_field(name=f"You rolled {dice[0][0]}d{die_type}",
                                value=f"which became {good_roll_text} {roll}", inline=False)
            else:

                embed.add_field(name=f"You rolled {dice[0][0]}d{dice[0][1]}", value=f"which became {roll}",
                                inline=False)
        else:
            sum_ = 0
            rolls = []
            ...
            for die in dice:
                ...
                if s:
                    embed.add_field(name=f"you also rolled {die[0]}d{die[1]}",
                                    value="which became " + '+'.join(rolls[-1]), inline=False)
                else:
                    embed.add_field(name=f"You rolled {dice[0][0]}d{dice[0][1]}",
                                    value="which became " + '+'.join(rolls[-1]), inline=False)
                    s = 1
