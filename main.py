...
async def encounter(level, size):
	if level == "1": 	
		if size == "2":
			...
			embed.set_author(name="Encounter for party level: " + ', '.join(level) +  " and, " + ', '.join(size) + " party members")
	...
	if level == "2":
		...
		if size == "3":
			...
			enc2 = random.choice(cr1)
			await client.say("You encounter: " + str(enc) + ", or you encounter: " + str(enc2))
		if size == "4":
