...
class taverncog():
	...
	async def rule(self, number):
		...
		async def faq(self, number):
			...
			if number == "2":
				embed.add_field(
					name="2. Where do I find games? When is the next game? How often do we run games? - #faq",
			...
			if number == "5":
				embed.add_field(name="5. Where do I find [thing]? What is [channel] for? - #faq",
								value="For a list of all the channels and their purpose, go to #tavern-menu.",
								inline=False)
